...
from util.input_parsing import get_d_from_H
...
def main():
    ...
    p.add_argument("--H_name", type=str, required=True)
    ...
    if args.H_name == "quantum_group_XXZ":
        H_local = get_XXZ_dmrg(args.module_name, args.q, args.n_irreps)
        dmrg.H = [H_local for i in range(dmrg.N)]
        dmrg.d = get_d_from_H(H_local)

    elif args.H_name == "A4_XXZ":
        labels_file, values_file, size_file = dataset_files(args.module_name)
        missing = [str(p) for p in (labels_file, values_file, size_file) if not p.exists()]
        if missing:
            print(json.dumps({"error": True, "reason": "missing_data_files",
                            "module_name": args.module_name, "missing": missng}))
            raise SystemExit(2)
        dmrg.H, dmrg.d = dmrg.get_H_and_d_from_files(dmrg.N, str(labels_file), str(values_file), str(size_file))
    
    else:
        print("No such Hamiltonian available")
        raise SystemExit(2)
    ...
    for _ in range(args.n_sweeps):
        ...
        for i in range(dmrg.N - 2, -1, -1):
            ...
            if args.save_entanglement_spectra:
                if int(i) == int(dmrg.N / 2):
                    S = dmrg.optimise_site_pair(i, is_moving_right=False, save_entanglement_spectra=True)
                    for i in range(len(S)):
                        S[i] = np.log(S[i])

                    plt.scatter(range(len(S)), sorted(S, reverse=True))
