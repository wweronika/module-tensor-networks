...
def run_one(python_bin, env, N, module, left_json, right_json, n_sweeps, out_fh, save_entanglement_spectra):
    """Run one dmrg_cli invocation and append its JSON (or error JSON) to out_fh."""
    cmd = [
        python_bin, "-m", "experiments.dmrg_cli",
        "--N", str(N),
        ...
        "--save_entanglement_spectra", str(save_entanglement_spectra),
        "--H_name", H_NAME
    ]
...
def main():
    ...
    ap.add_argument("--save_entanglement_spectra", type=bool, default=1, help="Flag for saving entanglement spectra")
    args = ap.parse_args()
    ...
    left_json = json.dumps([[0, 0]])  # <- fixed left BC
            ...
            for module in args.modules:
                ...
                for i in range(K):
                    ...
                    run_one(args.python_bin, env, N, module,
                            ...
                            args.save_entanglement_spectra)
